...
output_hh_seed_df <- hh_seed_df %>%
  ...
  select(RT, SERIALNO, DIVISION, PUMA, REGION, ST, ADJINC, WGTP, NP,
         ...
         county_index = COUNTY, unique_hh_id, weight, hhsize, incomeQ, workers, hhgqtype)
# remove NA
...
dict_df <- tibble(file = c("household_seed",
                  ...
                  definiton = c("PUMS expansion factor for household record",
                                ...
                                "Non-institutional group quarters living arrangement (1=university, 2=military, 3=other non-institutional, 0=in household)",
                                # marginals
...
# Write
```{r write}
write_csv(dict_df, file = paste0(output_data_dictionary_file_name, ".csv"))
write_feather(dict_df, sink = paste0(output_data_dictionary_file_name, ".feather"))

write_csv(output_hh_seed_df, file = paste0(output_household_seed_file_name, ".csv"))
write_feather(output_hh_seed_df, sink = paste0(output_household_seed_file_name, ".feather"))

write_csv(output_person_seed_df, file = paste0(output_person_seed_file_name, ".csv"))
write_feather(output_person_seed_df, sink = paste0(output_person_seed_file_name, ".feather"))

write_csv(output_marginals_df, file = paste0(output_marginal_database_file_name, ".csv"))
write_feather(output_marginals_df, sink = paste0(output_marginal_database_file_name, ".feather"))

write_csv(output_geo_df, file = paste0(output_geography_file_name, ".csv"))
